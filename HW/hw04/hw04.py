...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, answer):
        nonlocal balance, cnt, lst
        if cnt == 3:
            # print("\"Your account is locked. Attempts: ['%s', '%s', '%s']\"" %(lst[0], lst[1], lst[2]))
            return f"Your account is locked. Attempts: {lst}"
            # return  "Your account is locked. Attempts: ['%s', '%s', '%s']" %(lst[0], lst[1], lst[2])
        if answer != password:
            cnt += 1
            lst.append(answer)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        n = Mint.current_year - self.year - 50
        if n > 0:
            return self.cents + n
        else:
            return self.cents
...
class VendingMachine:
    ...
    def __init__(self, name, cost):
        self.name = name
        self.cost = cost
        self.current_num = VendingMachine.current_num
        self.money = 0
    ...
    def vend(self):
        if self.current_num == 0:
            ...
            if self.money < self.cost:
                dvalue = self.cost - self.money
                return 'You must deposit $%d more.' %(dvalue)
    ...
    def restock(self, num):
        ...
        return 'Current %s stock: %d' %(self.name, self.current_num)
